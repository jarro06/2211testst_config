...
def job_config = [
        job: [
                name: "2211testst_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: '2211testst_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/2211testst_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.2211testst.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "2211testst", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("2211testst_Pipeline_develop");

job.with {

    authenticationToken('Xkc6JL04fujVC349c6aygQRxPbes5ulk')
